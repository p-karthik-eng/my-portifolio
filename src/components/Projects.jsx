    ...
    <div className="relative bg-[#0f0b1d] min-h-[350px] flex flex-col items-center justify-center px-4 sm:px-8 md:px-14 py-12 md:py-56 rounded-2xl shadow-xl space-y-20 md:space-y-28 lg:space-y-56 overflow-hidden font-preahvihear mt-">
      ...
      {projectData.map((project, index) => (
        ...
        >
          {/* Left Section */}
          <div
            className={`flex flex-col space-y-4 md:w-3/5 relative z-10 ${
              index % 2 === 0
                ? "text-left md:text-left md:-mr-96"
                : "text-right md:text-right md:-ml-[20%]"
            }`}
          >
            ...
            <div className="text-gray-300 text-sm sm:text-base leading-relaxed bg-white bg-opacity-10 backdrop-blur-lg border border-white/20 rounded-lg p-4 w-[110%]">
              {project.description}
            </div>
